  ...
  return (
    <div className="flex flex-col md:flex-row items-center mb-12">
      ...
      <div className={`flex-1 p-6 ${textAlignment}`}>
        <h3 className="text-3xl font-bold mb-4 text-custom-purple">{heading}</h3>
        <ul className="list-none space-y-2">
          {listItems.map((item, index) => (
            <li key={index} className="flex items-center text-lg text-gray-600">
              <CheckCircleIcon className="w-6 h-6 text-[#6168c6] mr-2" />
              {item}
            </li>
          ))}
        </ul>
        <Link to="/SignIn">
          <button className="block bg-custom-purple text-white px-6 py-3 rounded-lg transition duration-300 hover:bg-[#262ea2] mt-6 mx-auto">
